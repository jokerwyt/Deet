        ...
        if let Some(program) = unit.line_program.clone() {
            ...
            while let Some((header, row)) = rows.next_row()? {
                if !row.end_sequence() {
                    ...
                    //     .iter_mut()
                    //     .find(|f| {
                    //         f.name == path.as_os_str().to_str().unwrap()
                    //     });
                    
                    // The line above doesn't work, maybe some compatibility problem.
                    // Dirty hack for one file case
                    let file = compilation_units.first_mut();
